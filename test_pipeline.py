...
def run_tests():
    ...
    logger.info(
        f"Data Ingestion OK: Ingested {len(df)} rows for NVDA. Latest Close: ${df['Close'].iloc[-1]:.2f}"
    )
    ...
    logger.info(
        f"L2 Order Book OK: Spread={order_book['spread_bps']} bps, Imbalance={order_book['imbalance']:+.2%}"
    )
    ...
    logger.info(
        f"Features OK: Generated {len(df_features.columns)} feature columns. Latest RSI: {df_features['RSI'].iloc[-1]:.1f}"
    )
    ...
    assert "lower_95" in arima_res and "upper_95" in arima_res, (
        "ARIMA missing confidence intervals!"
    )
    logger.info(
        f"ARIMA OK: Horizon=7, Expected Change={arima_res['expected_change_pct']:+.2f}%, AIC={arima_res['aic']}"
    )
    ...
    logger.info(
        f"ML Ensemble OK: Prob Bullish={ml_res['prob_bullish']}%, Directional Accuracy={ml_res['directional_accuracy']}%"
    )
    ...
    logger.info(
        f"Consensus Signal OK: {signal['action']} (Score: {signal['score']:+.1f}, SL: ${signal['stop_loss']}, TP: ${signal['take_profit']})"
    )

    risk_stats = QuantitativeRiskEngine.calculate_risk_metrics(df_features["Return"])
    assert "hist_var_95_pct" in risk_stats and "sharpe_ratio" in risk_stats, (
        "Risk calculation failed!"
    )
    logger.info(
        f"Risk Engine OK: Sharpe={risk_stats['sharpe_ratio']}, Ann Vol={risk_stats['ann_volatility_pct']}%, VaR 95%={risk_stats['hist_var_95_pct']}%"
    )
    ...
    logger.info("=== [7/7] TESTING ENTERPRISE QUANTSERVICE FACADE ===")
    from schemas.analytics import FullAnalysisResult
    from services.quant_service import QuantService

    service = QuantService(data_pipeline=pipeline)
    ...
    assert isinstance(analysis, FullAnalysisResult)
    assert analysis.execution_time_ms > 0
    logger.info(
        f"QuantService OK: Completed parallel analysis in {analysis.execution_time_ms:.1f}ms for {analysis.symbol}"
    )

    logger.info("ALL 7 ENTERPRISE SUITES PASSED CLEANLY!")
