...
def create_candlestick_chart(df: pd.DataFrame, symbol: str) -> go.Figure:
    ...
    if "EMA_9" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["EMA_9"], line=dict(color="#00f2fe", width=1.2), name="EMA 9"), row=1, col=1)
    if "EMA_21" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["EMA_21"], line=dict(color="#ffb300", width=1.2), name="EMA 21"), row=1, col=1)
    if "SMA_50" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["SMA_50"], line=dict(color="#38bdf8", width=1.2, dash="dash"), name="SMA 50"), row=1, col=1)

    if "BB_Upper" in df.columns and "BB_Lower" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["BB_Upper"], line=dict(color="rgba(255, 255, 255, 0.15)", width=1), showlegend=False), row=1, col=1)
...
def create_forecast_cone_chart(
    ...
    fig.add_hline(y=last_price, line_dash="dash", line_color="#64748b", annotation_text=f"Anchor: ${last_price:.2f}")
    ...
    fig.update_layout(
        ...
        hovermode="x unified",
        height=450,
        title=dict(text=f"{symbol} Predictive Trajectory & Confidence Cones", font=dict(size=14, color="#94a3b8"))
    )
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor="#182234")
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor="#182234")
...
def create_oscillators_chart(df: pd.DataFrame) -> go.Figure:
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.08, subplot_titles=("MACD (12, 26, 9)", "RSI (14)"))

    if "MACD" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["MACD"], line=dict(color="#00f2fe", width=1.5), name="MACD"), row=1, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=df["MACD_Signal"], line=dict(color="#ffb300", width=1.5), name="Signal"), row=1, col=1)
        hist_colors = ["#00e676" if h >= 0 else "#ff1744" for h in df["MACD_Hist"]]
        fig.add_trace(go.Bar(x=df.index, y=df["MACD_Hist"], marker_color=hist_colors, name="Hist"), row=1, col=1)

    if "RSI" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["RSI"], line=dict(color="#a855f7", width=1.8), name="RSI"), row=2, col=1)
...
def create_risk_distribution_chart(returns_series: pd.Series, risk_stats: Dict[str, Any]) -> go.Figure:
    ...
    fig.update_layout(
        ...
        margin=dict(l=10, r=10, t=30, b=20),
        height=320,
        title=dict(text="Empirical Returns Distribution & Tail Risk Cutoffs", font=dict(size=13, color="#94a3b8")),
        showlegend=False
    )
...
def create_drawdown_chart(dd_series: pd.Series) -> go.Figure:
    ...
    fig.update_layout(
        ...
        margin=dict(l=10, r=10, t=30, b=20),
        height=300,
        title=dict(text="Historical Underwater Drawdown Curve (%)", font=dict(size=13, color="#94a3b8")),
        showlegend=False
    )
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor="#182234")
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor="#182234")
