...
class MarketDataPipeline:
    ...
    def fetch_historical_data(
        ...
        logger.warning(f"Live ingestion unavailable for {symbol}. Generating synthetic market microstructure.")
        synthetic_df = self._generate_synthetic_historical_data(symbol, period=period, interval=interval)
        self._cache[cache_key] = (now, synthetic_df)
        return synthetic_df.copy()

    def fetch_live_quote(self, symbol: str) -> Dict[str, float]:
        """
    ...
    def fetch_batch_ticker_tape(self, symbols: List[str]) -> List[Dict[str, float]]:
        quotes = []
        for sym in symbols:
    ...
    def generate_order_book_depth(
        ...
        imbalance_ratio = (total_bid_depth - total_ask_depth) / (total_bid_depth + total_ask_depth + 1e-6)
    ...
    def _generate_synthetic_historical_data(
        ...
        high_spread = np.abs(np.random.exponential(scale=daily_vol * 0.7, size=n_bars)) * price_series
        low_spread = np.abs(np.random.exponential(scale=daily_vol * 0.7, size=n_bars)) * price_series
